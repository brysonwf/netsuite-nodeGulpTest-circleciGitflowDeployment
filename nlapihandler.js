...
function nlapiLoadRecord(type, id, initializeValues) {
  var nlobjRecordVar = new nlobjRecord();

  //insert custom rest endpoing or test data.

  return nlobjRecordVar;
}
...
function nlapiRequestURL(url, postdata, headers, callback, method) {
  return new nlobjServerResponse();
}
...
function nlapiGetFieldValue(fldnam) {
  ...
  if (fldnam == 'custbody_isshopify'){
    return 'T';
  }

  if (fldnam == 'orderstatus'){
    return 'T';
  }

  if (fldnam == 'custbody_release_to_warehouse'){
    return 'F';
  }

  if (fldnam == 'custbody_cash_sale_transaction'){
    return '12345';
  }
...
function nlobjServerResponse( ) {
  this.constructorVar = 'constructor';
}
...
nlobjServerResponse.prototype.getBody = function( ) {
  return nlobjServerResponseprototypegetBody;
}
 ...
 * @since 2012.2
 */
nlobjResponse.prototype.getCode = function() { return '200'; }
